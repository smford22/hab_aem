...
resource "aws_vpc" "default" {
  ...
  tags {
    Name = "${var.aws_key_pair_name}_${random_id.aem_id.hex}_aem"
  }
}
...
resource "aws_security_group" "aem" {
  name        = "${var.aws_key_pair_name}-${random_id.aem_id.hex}-aem"
...
resource "aws_instance" "initial-peer" {
  ...
  vpc_security_group_ids      = ["${aws_security_group.aem.id}"]
  associate_public_ip_address = true

  tags {
    Name      = "${var.aws_key_pair_name}_${random_id.aem_id.hex}_initial_peer"
    X-Dept    = "SCE"
    X-Contact = "${var.aws_key_pair_name} <maintainer@example.com>"
  }
...
resource "aws_instance" "aem_publisher" {
  ...
  tags {
    Name      = "${var.aws_key_pair_name}_${random_id.aem_id.hex}_aem_publisher"
    X-Dept    = "SCE"
    X-Contact = "${var.aws_key_pair_name} <maintainer@example.com>"
  }
...
resource "aws_instance" "aem_author" {
  ...
  tags {
    Name      = "${var.aws_key_pair_name}_${random_id.aem_id.hex}_aem_author"
    X-Dept    = "SCE"
